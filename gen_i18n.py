...
for locale in locales:
    ...
    with open(os.path.join(I18N_RC_DIR, "../splatnet", file_name), "w") as f:
        json.dump(splatnet, f, ensure_ascii=False)
    with open(os.path.join(I18N_RC_DIR, file_name), "r") as f:
        i18n = json.load(f)
    i18n.update({"splatnet": splatnet})
    with open(os.path.join(RUST_LOCALES_DIR, file_name), "w") as f:
